-- =========================================================
-- FITUR KARTU PENDAFTARAN + PENGUMUMAN + NOTIFIKASI
-- Jalankan setelah schema/SELEKSI/NILAI sudah terpasang.
-- =========================================================
...
create index if not exists notifikasi_calon_idx on public.notifikasi(calon_siswa_id);
...
create policy "student read own notifications" on public.notifikasi
for select to authenticated
using (user_id = auth.uid() or public.is_admin());

create policy "student update own notifications" on public.notifikasi
for update to authenticated
using (user_id = auth.uid())
with check (user_id = auth.uid());

create policy "admin manage notifications" on public.notifikasi
for all to authenticated
using (public.is_admin()) with check (public.is_admin());

-- Saat admin menerbitkan pengumuman, semua akun siswa mendapat notifikasi.
    ...
    where p.role = 'student'
      and not exists (
        ...
        where n.user_id = p.id
          and n.judul = 'Pengumuman Baru: ' || new.judul
          and n.created_at >= new.created_at - interval '1 minute'
      );
  end if;
  return new;
end;
...
create trigger trg_notifikasi_pengumuman
after insert or update of published on public.pengumuman
for each row execute function public.kirim_notifikasi_pengumuman();

-- =========================================================
-- PROSES SELEKSI + NOTIFIKASI HASIL
-- Nilai akhir tetap dari trigger Rapor 60% + Tes 40%.
-- =========================================================
...
begin
  ...
  -- Hapus notifikasi hasil seleksi lama agar hasil terbaru tidak menumpuk.
  delete from public.notifikasi where tipe = 'hasil_seleksi';

  for calon in
    select id, user_id, nama, pilihan_jurusan_1_id, pilihan_jurusan_2_id, jurusan_id, nilai, created_at
  ...
  loop
    ...
    if pilihan1 is not null then
      ...
      from public.calon_siswa
      where jurusan_diterima_id = pilihan1 and status_seleksi = 'diterima';
    ...
    if diterima is null and pilihan2 is not null and pilihan2 <> pilihan1 then
      select count(*) into kuota_terpakai
      from public.calon_siswa
      where jurusan_diterima_id = pilihan2 and status_seleksi = 'diterima';
    ...
    if diterima is not null then
      update public.calon_siswa
      set jurusan_diterima_id = diterima, status_seleksi = 'diterima', status = 'Diterima'
      where id = calon.id;
      select nama into nama_jurusan from public.jurusan where id = diterima;
    else
      update public.calon_siswa
      set status_seleksi = 'tidak_diterima', status = 'Ditolak'
      where id = calon.id;
      nama_jurusan := null;
    end if;
    ...
    if calon.user_id is not null then
      ...
      values (
        calon.user_id,
        calon.id,
        case when diterima is not null then 'Hasil Seleksi SPMB: Diterima' else 'Hasil Seleksi SPMB: Belum Diterima' end,
        case when diterima is not null
          then 'Selamat ' || calon.nama || ', kamu diterima di jurusan ' || coalesce(nama_jurusan, '-') || '. Peringkat #' || peringkat_counter || ' dengan nilai akhir ' || to_char(calon.nilai, 'FM990.00') || '.'
          else 'Hasil seleksi sudah tersedia. Kamu belum diterima pada pilihan jurusan yang tersedia. Peringkat #' || peringkat_counter || ' dengan nilai akhir ' || to_char(calon.nilai, 'FM990.00') || '.'
        end,
        'hasil_seleksi'
      );
    end if;
  ...
  return json_build_object(
    'success', true,
    'message', 'Seleksi berhasil diproses dan notifikasi hasil telah dikirim ke akun siswa.'
  );
end;
$$;
...
-- Pastikan fungsi cek status terbaru menggunakan NISN.
